...
function upgrade() {
    if (nowMoney >= buttons[nowButton + 1].cost) {
        ...
        nowButton ++;
        if (nowButton == 1) {
            document.body.style.backgroundImage = ` url(images/흙.jpg)`;
        }
    } else {
        alert('파기 실패');
    }
...
function fever() {
    ...
    if (nowFeverMain >= 100) {
        nowMoney += 2*mouses[nowMouse].cash 
        nowFeverMain = 0;
    }
}
